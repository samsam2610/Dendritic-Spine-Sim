...
#-------------------------------------------------------------------------------
# Add tDCS extracellular field model
#-------------------------------------------------------------------------------
FIELD_AREA_MM2 = 3 * 3        # mm^2
FIELD_AREA_M2 = FIELD_AREA_MM2 * 1e-6  # convert to m^2
CURRENT = 6e-6                # 6 µA
SIGMA_TISSUE = 0.3            # S/m, brain conductivity (gray matter)
FIELD_STRENGTH = CURRENT / (SIGMA_TISSUE * FIELD_AREA_M2)  # V/m
FIELD_DIRECTION = [0, 1, 0]   # assume field points along y (depth)

if cfg.addTDCS:
    params = cfg.tDCS
    amp = params['amp']
    direction = params.get('dir', [0, 1, 0])
    ynormRange = params.get('ynormRange', [0.0, 1.0])

    netParams.stimSourceParams['tDCS'] = {
        'type': 'tDCS',
        'amplitude': amp,
        'direction': direction,
        'ynormRange': ynormRange
    }
#------------------------------------------------------------------------------
# NetStim inputs
#------------------------------------------------------------------------------
...
- Added tDCS extracellular field model
"""
